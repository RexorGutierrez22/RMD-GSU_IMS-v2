...
class BackupDatabase extends Command
{
     ...
     * @var string
     */
    protected $description = 'Create a database backup only';
    ...
    public function handle()
    {
        ...
        try {
            ...
            // Run the backup command with only database option
            ...
            if (strpos($output, 'success') !== false || strpos($output, 'completed') !== false || strpos($output, 'Backup completed') !== false) {
                ...
                Log::warning('Database backup completed with warnings', [
                    'output' => $output,
                    'timestamp' => Carbon::now()->toDateTimeString(),
                ]);
                return 0;
            ...
            if (strpos($e->getMessage(), 'notification') !== false) {
                try {
                    ...
                    if (strpos($output, 'Backup completed') !== false || strpos($output, 'success') !== false) {
                        $this->info('✅ Database backup completed successfully (notifications disabled)');
                        Log::info('Database backup completed successfully (notifications disabled)', [
                            'timestamp' => Carbon::now()->toDateTimeString(),
                        ]);
                        return 0;
                    }
